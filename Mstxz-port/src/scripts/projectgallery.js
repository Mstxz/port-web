...
function loadSpotlightedGame(){
    Promise.all(
        ...
        let project = document.getElementById('spotlight');
...
function loadSpotlightedMusic() {
  Promise.all([
    fetch('/data/musictrack.json').then(r => r.json()),
    fetch('/components/tracklist.html').then(r => r.text())
  ]).then(([data, comp]) => {
    const project = document.getElementById('spotlight');
    project.innerHTML = "";
